'use strict';

import { assert } from 'chai';
import utils from './utils';
import ApiControllers from '../lib/apicontrollers';
import { CreateTransactionRequest, CreateTransactionResponse, MessageTypeEnum, OpaqueDataType, PaymentType, TransactionRequestType, TransactionTypeEnum } from '../lib/types';

class ApplePayTestData {
	merchantAuthentication = utils.merchantAuthentication;
	op: OpaqueDataType = {
		dataDescriptor: 'COMMON.APPLE.INAPP.PAYMENT',
		dataValue: ''
	}
	paymentOne: PaymentType = {
		opaqueData: this.op
	}
}

describe('Apple Pay', function() {
	this.timeout(120000);
	var testData = new ApplePayTestData();

	describe.skip('Create an Apple Pay Transaction', function () {
		var response: CreateTransactionResponse

		before(function(done){
			var transactionRequest: TransactionRequestType = {
				transactionType: TransactionTypeEnum.AUTHCAPTURETRANSACTION,
				amount: utils.getRandomAmount(),
				payment: testData.paymentOne,
			}
			var createRequest: CreateTransactionRequest = {
				merchantAuthentication: testData.merchantAuthentication,
				transactionRequest,
			};
				
			var ctrl = new ApiControllers.CreateTransactionController(createRequest);

			ctrl.execute(function(){
				var apiResponse = ctrl.getResponse();
				response = (apiResponse as unknown) as CreateTransactionResponse;
				done();
			});
		});

		it('should return resultcode Ok when successful', function () {

			assert.equal(response.messages.resultCode, MessageTypeEnum.OK);
		});
	});
});
